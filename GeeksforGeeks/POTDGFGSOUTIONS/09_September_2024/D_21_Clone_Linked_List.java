...
class Solution {
    ...
    Node copyList(Node head) {
        ...
        while (temp != null) {
            ...
            temp.next = newNode;
            temp = newNode.next; // Move to the next original node
        }

        // STEP 2: Assign random pointers to the cloned nodes
        temp = head;
        Node head2 = head.next; // This will be the head of the cloned list
        while (temp != null) {
            if (temp.random != null) {
                temp.next.random = temp.random.next; // Link the random pointer
            }
        ...
        temp = head;
        while (temp != null) {
            Node temp2 = temp.next; // Get the cloned node
            temp.next = temp2.next; // Restore the original list's next pointer
            if (temp2.next != null) {
                temp2.next = temp2.next.next; // Link the cloned nodes
            }
            temp = temp.next; // Move to the next original node
        }

        return head2; // Return the head of the cloned list
    }
}
